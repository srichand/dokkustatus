...
final class StatusStore: ObservableObject {
    ...
    init(
        ...
        self.hostConfig = configStore.loadActive() ?? defaultProvider.loadDefaultConfig()
    ...
    func refreshNow() async {
        ...
        do {
            ...
            apps = statuses
            lastCheckedAt = Date()
            latestError = collapsedAppError(from: statuses)
            aggregateState = AggregateState.evaluate(apps: statuses, latestError: latestError)
        } catch {
